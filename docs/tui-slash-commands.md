...
| `/experimental` | — | Open the experimental-feature submenu showing the catalog and which flags are enabled this session (`dispatch`, `lsp_code_intelligence`, …). `background_subagents` has graduated to GA — the flag is now a recognized no-op. Read-only; enabling happens via `--experimental <name>`, `YOTTACODE_EXPERIMENTAL`, or the `[experimental]` config block — see [experimental.md](experimental.md). |
...
| `/yolo` | — | Toggle yolo mode — NO safety floor; deny rules still win. See [Yolo mode](#yolo-mode). |
