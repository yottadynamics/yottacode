...
| `/experimental` | — | List active experimental features and graduated compatibility flags. Graduated entries such as `background_subagents` show as GA/no-op; active experiments such as `dispatch` and `lsp_code_intelligence` show ON/off. Read-only; enabling active experiments happens via `--experimental <name>`, `YOTTACODE_EXPERIMENTAL`, or the `[experimental]` config block — see [experimental.md](experimental.md). |
