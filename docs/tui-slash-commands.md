...
| `/map` | `[query]` | Open the experimental code map. `/map here [path]` deterministically suggests up to 8 high-signal changed or related files (and may return fewer) and explains why each belongs; `a` inserts all suggestions as `@path` prompt refs, while Enter inserts the selected file/symbol. Other modes stay under the same command: `/map deps <path>`, `/map dependents <path>`, `/map impact [--depth N\|all] <path>`, `/map cycles [path]`, and `/map diagram [path]`. Enable with `--experimental code_map`; this also adds model guidance to use the indexed tools and verify relevant results against source before editing. |
...
| `/context` | — | Show context-window diagnostics: resolved window, thresholds, tool-schema overhead, largest buckets, compaction status, and MCP/memory/skills breakdowns. A `Working set` section lists the latest turn's attached `@path` references. |
