...
| `/inspect` | `[session-id]` | Open a read-only, scrollable turn-by-turn session replay. Omit the id for the current session, or pass a full session id/name or a unique short id from `/usage`'s today rollup. Shows each user prompt preview, assistant preview, tool calls with truncated args, tool outcomes, per-turn tokens, and low-signal markers without switching the live conversation. |
