...
| `/map` | `[query]` | Open the experimental code map. `/map here` shows a ranked suggested-context list — changed files, their direct import neighbors, likely tests, and likely docs — with `a` to attach every listed file as an `@path` ref in one keystroke; Enter inserts one selected file/symbol the same way. Filtering to an exact directory (e.g. `/map internal/codemap`) renders a subsystem overview (entry points, public surface, core types, tests, key dependencies) instead of a flat list. Other modes stay under the same command: `/map deps <path>`, `/map dependents <path>`, `/map impact [--depth N\|all] <path>` (proximity-ranked, includes likely tests/docs), `/map cycles [path]`, and `/map diagram [--export <path>] [path]` (`--export` writes the full, untruncated diagram to disk). Enable with `--experimental code_map`. |
