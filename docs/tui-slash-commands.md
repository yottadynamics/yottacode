...
| `/router` | `[on\|off]` | Configure cache-safe model routing between a fast and a smart model. Bare opens a picker with four rows — Routing, Smart model, Fast model, Smart fallback — that all act **in place** (the picker stays open). The Routing row toggles on/off; the model rows open a list of your configured models (catalog + `providers.models`). You can enable routing first and pick the models below — routing turns on automatically once both are set — or pick the models and then toggle on. Choices **persist to `config.toml`** and apply live; picking a catalog model also adds it to that provider's `providers.models` so the write validates; configuring a new smart model also switches the active conversation model to it on close (announced in scrollback, same as `/model`). `on`/`off` are quick shortcuts for the toggle (also persist); `on` always means `auto` (summarization → fast model; every delegated subagent → smart model, unless it pins an explicit `model:`). The main-thread model is never switched mid-conversation, so routing only ever touches isolated contexts and never invalidates the prompt cache — a pure saving. In auto, the status bar's primary segment becomes the `<smart>:<fast>` pair while the active model matches the smart slot; if you later `/model` elsewhere, the bar shows the real active model with a dim `routing: auto (…)` note. `manual` mode (route only subagents with explicit `model:` frontmatter) stays config-only and shows a dim `routing: manual` note. |
