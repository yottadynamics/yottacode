...
| `/lsp` | `[path]` | Show LSP language detection, server status, and install hints for the current workspace or path. Requires `lsp_code_intelligence` — see [lsp.md](lsp.md). |
| `/health` | — | Show repo, provider, memory, LSP, MCP, and experimental state in one card. |
| `/config-doctor` | — | Validate local config dependencies such as LSP commands, MCP commands, and memory embeddings. |
| `/handoff` | — | Print a concise session handoff-note template for continuing work later. |
