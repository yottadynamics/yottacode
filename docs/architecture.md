...
During active TUI turns, streamed reasoning and provisional answer text share a
live preview above the cmdline only while there is content to show. Tool-only
work does not reserve blank preview rows, so the plan card and thinking row stay
compact while fresh transcript output continues to follow the bottom unless the
user intentionally scrolls away.
...
explicit queue receipt instead of silently discarding the draft. Approval
modals also clamp long command previews to the terminal height and keep the
