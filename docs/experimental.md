...
| `browser` | experimental | The `browser_status`/`browser_navigate`/`browser_screenshot`/`browser_inspect`/`browser_click`/`browser_type`/`browser_hotkey`/`browser_scroll`/`browser_wait`/`browser_close`/`browser_tabs`/`browser_switch_tab`/`browser_close_tab`/`browser_upload`/`browser_download`/`browser_console_logs`/`browser_network_requests`/`browser_eval`/`browser_back`/`browser_dialog` tools — drive a real, headless Chrome/Chromium instance over CDP via `go-rod/rod`, no Node.js/Playwright. A fresh isolated profile per session (never your real, logged-in browser); headless only; interactive elements carry `@eN` refs every tool accepts, iframes included; the backend can be swapped (a rented cloud browser, a self-hosted Camofox server) and stealth/proxy enabled under `[browser]` — see [configuration.md](configuration.md#browser); the session tracks every tab it opens but only ever acts on one active tab at a time (`browser_click`/`browser_type` auto-follow a tab their own action opens; `browser_tabs`/`browser_switch_tab` cover the rest); not available to `dispatch` workers. See [browser.md](browser.md) for what to use it for and worked examples, [tools.md](tools.md#browser_status) for the per-tool reference, and [security-and-allow-lists.md](security-and-allow-lists.md#browser-automation) for the safety model. |
...
| `code_map` | experimental | Repository structure map, all 5 roadmap phases shipped. Adds the `/map` TUI overlay and `code_map`, `code_symbols`, `code_structure_projection`, `code_dependencies`, `code_dependents`, `code_impact`, `code_cycles`, and `code_map_diagram` agent tools — all read-only except `code_structure_projection`/`code_map_diagram`'s optional `to_file` export, which writes like any other tool (approval-gated, checkpoint-tracked). The TUI includes a ranked `/map here` suggested-context list (with `a` to attach it all), a subsystem overview when filtering to an exact directory, and structure, dependency, proximity-ranked impact, cycle, and Mermaid diagram modes. The index is LSP-backed when available, falls back to approximate regex symbols, and is watcher-backed (`fsnotify`): a changed file is incrementally re-derived and patched into the persistent graph rather than triggering a full workspace re-walk, with a safe fingerprint-walk fallback for oversized repos or watch setup failures. Import-edge resolution covers Go, TypeScript/JavaScript, Python, and Rust — including Rust `self::`/`super::` module-tree resolution; `code_impact`'s `include_calls` best-effort supplements with live LSP call hierarchy. See [code-map.md](code-map.md). |
...
| `syntax_ranges` | **graduated** (GA) | The read-only `syntax_range` agent tool is now default-on for Go, TypeScript/JavaScript, Python, and Rust. Go uses its AST parser; the other families use conservative structural scanners. Results include canonical kinds, exact half-open byte spans, hashline-compatible receipts, and recoverable warnings. The flag is a no-op kept for one release so existing configs don't break. See [tools.md](tools.md#syntax_range). |
