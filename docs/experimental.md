...
| `document_ingestion` | experimental | The `read_document` agent tool — bounded, provenance-labeled text extraction for CSV, TSV, JSON, JSONL, XML, and HTML files, a structured alternative to `read_file` for these formats. Still opt-in because the default caps haven't been exercised against real-world exports and format coverage is unfinished (PDF and Office formats are explicitly out of scope for this phase). See [tools.md](tools.md#read_document). |
...
| `sandbox` | experimental | Routes `run_bash` through a session-scoped rootless podman container (`network=none`, project-dir-only mount, capability-dropped) instead of the host directly, when `[sandbox].backend = "podman"` in config. Opt-in first because the container-lifecycle and hardening-flag set haven't been exercised in production yet. See [sandbox.md](sandbox.md). |
