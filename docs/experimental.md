...
| `lsp_code_intelligence` | experimental | Language-server-backed code intelligence tools (`lsp_status`, `lsp_symbols`, `lsp_definition`, `lsp_references`, `lsp_diagnostics`, `lsp_changed_files_diagnostics`, `lsp_hover`, `lsp_code_actions`, `lsp_call_hierarchy`, preview/apply WorkspaceEdit tools). Detects supported languages in the workspace and reports missing server binaries with install hints. Servers are never installed automatically; install `gopls`, `typescript-language-server`, `pyright-langserver`, or `rust-analyzer` on `PATH` for the languages you use. See [lsp.md](lsp.md). |
