...
| `document_ingestion` | experimental | The `read_document` agent tool — bounded, provenance-labeled text extraction for CSV, TSV, JSON, JSONL, XML, and HTML files, a structured alternative to `read_file` for these formats. Caps (byte/row/char) and format coverage (PDF, Office formats are explicitly out of scope for this phase) are still settling. See [tools.md](tools.md#read_document). |
