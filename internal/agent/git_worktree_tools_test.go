...
func TestGitWorktreeTools(t *testing.T) {
	...
	listTool := &GitWorktreeListTool{Cwd: NewCwdRef(repoPath)}
	...
	addTool := &GitWorktreeAddTool{Cwd: NewCwdRef(repoPath)}
	...
	lockTool := &GitWorktreeLockTool{Cwd: NewCwdRef(repoPath)}
	...
	unlockTool := &GitWorktreeUnlockTool{Cwd: NewCwdRef(repoPath)}
	...
	pruneTool := &GitWorktreePruneTool{Cwd: NewCwdRef(repoPath)}
	out, err = pruneTool.Execute(ctx, "{}")
	require.NoError(t, err)
	require.Contains(t, out, "pruned stale worktree data")

	// Test GitWorktreeRemoveTool
	removeTool := &GitWorktreeRemoveTool{Cwd: NewCwdRef(repoPath)}
	...
	removeTool2 := &GitWorktreeRemoveTool{Cwd: NewCwdRef(repoPath)}
	_, err = removeTool2.Execute(ctx, `{"path": "`+worktreePath2+`"}`)
	require.NoError(t, err)
	removeTool3 := &GitWorktreeRemoveTool{Cwd: NewCwdRef(repoPath)}
	_, err = removeTool3.Execute(ctx, `{"path": "`+worktreePath3+`"}`)
	require.NoError(t, err)
