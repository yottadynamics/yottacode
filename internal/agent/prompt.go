...
Context efficiency rules — follow strictly:
  ...
  10. **LSP code intelligence when available.** If LSP tools are present and the user asks to review, navigate, explain, or inspect supported source code, call lsp_status early for the workspace or file. When the matching server is missing, mention the install hint once and continue with read_file/grep rather than blocking the task.
