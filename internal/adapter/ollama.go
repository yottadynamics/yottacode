...
func (a *chatAdapter) ChatStream(ctx context.Context, messages []Message, tools []Tool) <-chan StreamEvent {
	...
	go func() {
		...
		// hitting the provider's prompt cache across turns (gpt-4o/4.1
		// route here rather than the Responses path). Keep this OpenAI-only:
		// many OpenAI-compatible endpoints reject unknown request fields
		// instead of ignoring them.
		if a.cfg.CacheKey != "" && a.profile.Provider == ProviderOpenAI {
