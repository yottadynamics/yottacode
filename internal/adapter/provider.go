...
type Config struct {
	...
	// CacheKey is the stable per-conversation identifier sent as the
	// Responses-API `prompt_cache_key`. It pins every turn of a session
	// to the same server-side prompt-cache shard so the (byte-identical)
	// prompt prefix keeps hitting the KV cache instead of oscillating
	// across load-balanced shards — the same lever the official Codex
	// CLI pulls, which sets it to the conversation id. Populated from
	// session.Session.ID at construction. Empty omits the field, so a
	// session-less caller (e.g. the connection probe) sends the same
	// request shape as before. Consumed only by the Responses-API
	// adapters (openai-auth Codex backend + api.openai.com); the
	// Chat-Completions adapters ignore it.
	CacheKey string
