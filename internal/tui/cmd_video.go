...
3. If the goal references media recordings, call media_probe on each recording and use media_analyze for candidate cuts when cleanup is needed. When the goal combines multiple recordings, keep the first render desktop-safe: propose exact per-clip keep_ranges, use media_compose once to create a draft MP4, and stop for approval before any follow-up media_render pass.
...
6. After approval, use media_compose to assemble title cards, screenshots/images, and approved clip segments into a draft MP4. For multiple source videos, do not immediately chain a full media_render export after composition; return the draft path first so the user can inspect it, then render one final MP4 profile only after they approve the combined draft. Prefer branded templates, lower-third captions, simple fades, and image zoom/pan motion when they make the result clearer. Then use media_render for final YouTube/X/GIF profile exports when requested — render one profile first to confirm the draft looks right before rendering every requested profile, and don't default to "gif_preview_large" (the most CPU-intensive profile) unless the user specifically asks for it.
