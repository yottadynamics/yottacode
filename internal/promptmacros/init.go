...
// write to. Kept as a function so callers can swap it if we ever
// support alternate roots, and so the path appears once instead of
// being duplicated across the prompt body.
func ProjectMdRelPath() string {
	return filepath.Join(".yottacode", "YOTTACODE.md")
}

// BuildInitPrompt composes the directive sent to the agent for /init.
// Path-aware: when YOTTACODE.md already exists, the prompt asks the
// agent to refresh it rather than overwriting blindly, so a
// hand-edited file isn't flattened on a re-run. The approval modal
// still gates the write either way.
