...
func TestLoad_DeprecatedMemoryFinalTurnOnQuitIsAcceptedButNotRendered(t *testing.T) {
	cfg, err := Load(writeFile(t, "[memory]\nfinal_turn_on_quit = true\n"))
	if err != nil {
		t.Fatalf("legacy strict config must load: %v", err)
	}
	if rendered := Render(cfg); strings.Contains(rendered, "final_turn_on_quit") {
		t.Fatalf("deprecated key must not be emitted in new config: %s", rendered)
	}
}
